...
const fontSize = computed(() => {
  return yesButtonClickCount.value === 0
    ? 22
    : Math.max(16, 20 + yesButtonClickCount.value * 4);
});
...
const onYesButtonClick = () => {
  ...
  title.value = `Sabía que dirías que sí mi Nico 😍 Yaaay`;
...
<template>
  <section
    class="p-8 w-full h-full min-h-dvh items-center justify-center flex flex-col gap-0 md:p-16 md:gap-8"
  >
    <h1 class="text-center font-bold text-[clamp(26px,5vw,42px)] max-w-4xl">
      {{ title }}
    </h1>
    ...
    <div
      class="flex justify-center items-center flex-wrap w-full h-full gap-8 pt-0 md:pt-8"
      v-if="!isYesButtonClicked"
    >
      <Button
        type="button"
        label="Sí"
